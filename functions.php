...
set_post_thumbnail_size( 445, 320, true ); // Normal post thumbnails
add_image_size( 'blog-post-thumbnail', 445, 320 ); // Permalink thumbnail size
...
function twentyten_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<div id="comment-<?php comment_ID(); ?>" class="comment-a">
		<div class="comment-author vcard">
			<?php echo get_avatar( $comment, 40 ); ?>
			<?php printf( __( '%s <span class="says">says:</span>', 'twentyten' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
		</div><!-- .comment-author .vcard -->
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<small><?php _e( 'Your comment is awaiting moderation.', 'twentyten' ); ?></small>
			
		<?php endif; ?>

		<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				/* translators: 1: date(%1$s), 2: time(%2$s) */
...
function twentyten_posted_in() {
	...
	// Function that controls the length of the exerpts.
	function new_excerpt_length($length) {
		return 100;
	}
	add_filter('excerpt_length', 'new_excerpt_length');
	
	// Function to give it a container to style the read more link
	...
	// Function to limit our excerpts words
